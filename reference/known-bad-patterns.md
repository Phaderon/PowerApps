...
- **Hardcoding a Modern control's version number from this Bible without checking the live PA2105 warning.** Modern control versions (`ModernDropdown@x.x.x`, `ModernDatePicker@x.x.x`, etc.) drift upward silently as Microsoft ships updates to the maker environment — a version recorded here on one date can be stale a few weeks later. Confirmed: `ModernDatePicker@1.0.0` (recorded in `powerapps-control-defaults.md`/`yaml-validation-checklist.md`) threw `PA2108: Unknown property 'SelectedDate'` on paste, even though Microsoft's own docs list `SelectedDate` as a real property — the *declared* older version's schema didn't have it yet. The same paste's PA2105 warnings named the actual current version (`1.0.1` for DatePicker, `1.0.2` for Dropdown at time of writing) as plain warnings alongside the hard error, which is the giveaway: **if a PA2108 "unknown property" error fires on a property that's independently confirmed to exist for that control family, check for a PA2105 version-mismatch warning on the same control in the same paste and bump to the version it names, before assuming the property itself is wrong.** Policy Tracker `ViewItem_1`, 2026-07-05.
