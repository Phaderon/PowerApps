...
**Does not work:** `Base64(anyTextValue)` — Studio's formula bar rejects it outright: *"'Base64' is an unknown or unsupported function."* This function has no prior use anywhere else in this Bible either — treat any guide/example that calls a bare `Base64()` on a text value as broken, not just unverified.

**Confirmed fix:** `"data:image/svg+xml," & EncodeUrl(anyTextValue)` — the non-base64 data URI form, percent-encoded via `EncodeUrl()` instead — works. Confirmed live 2026-09-11 rendering a real card (rounded rect, fill, two text lines) built via Power Fx `&` concatenation, not just a hand-typed literal. Use this pattern, not `Base64()`, for any future "generated SVG in an Image control" build in this app family.
