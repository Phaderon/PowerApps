...
- [ ] **[YAML parser] `HtmlText` and other non-event-handler properties with inline CSS** — `|-` is blocked (PA1001) on these properties, so the block-scalar escape is not available. Use YAML **single-quote wrapping** instead: `HtmlText: '="<div style=""border: 1px solid ..."">"'`. Any literal single quotes inside (e.g. `'Open Sans'`) must be doubled: `''Open Sans''`. Triggers on any CSS property value (`border:`, `padding:`, `margin:`, `font-family:`, `font-size:`, etc.) embedded in a PowerFX string literal on a non-handler property.
