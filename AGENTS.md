...
**Screen YAML Vault — password-gated paste-and-save store for screen YAML, per app**
(added 2026-09-18). Every app on the hub has a `yaml.html` page (linked as "📋 Screen
YAML Vault" from that app's guide page topline) with a vertical-tabbed UI: two
app-wide tabs (App.OnStart, Formulas) pinned first, then one tab per screen. The
user pastes YAML/formulas straight from Power Apps Studio and hits Save — no more
notepad round-trips before handing content to an agent.
- **One central API, `functions/api/screen-yaml.js`, deployed only to
  `powerapps.pages.dev`** — backed by KV namespace `SCREEN_YAML` (id
  `8a49a1707de3403880d4e43bbecbcd76`) bound to the `powerapps` Pages project. Every
  app's `yaml.html` calls this one API over CORS (open the same way as
  `upload.js`), regardless of which repo or Cloudflare Pages project actually hosts
  that page — so no other Pages project needs its own KV namespace.
- One KV key per app (`app:<slug>`), value `{ order: [...], sections: { key: {
  label, content, updated } } }`. Every method, including GET, is password-gated
  (`UPLOAD_PASSWORD`, same secret as the upload box) because pasted YAML can contain
  real email addresses.
- **Read this directly instead of asking the user to re-paste:**
  `curl "https://powerapps.pages.dev/api/screen-yaml?app=<slug>&password=<UPLOAD_PASSWORD>"`
  — app slugs: `training-tracker`, `policy-tracker`, `branch-contact-groups`,
  `staff-movements`, `library-file-plan-manager`, `cadets-org-chart`.
- Front-end remembers the password in that browser's `localStorage` for 30 days
  (`pa-yaml-pass-<slug>` key) so the user doesn't have to re-enter it every visit —
  deliberately not a real cookie, same practical effect.
- Generated by `tools/gen-yaml-vault.js` (one template, per-app config array at the
  top). Re-run it after adding a new app or a new screen to an app's preset list —
  it only re-seeds the empty preset tabs shown before anything's saved; it never
  touches already-saved KV content. Deploy target files after regenerating:
  `training-tracker/yaml.html` and `cadets-org-chart/yaml.html` in this repo,
  `docs/yaml.html` in each of `PhadeDev/policy-tracker`, `PhadeDev/branch-contact-groups`,
  `PhadeDev/staff-movements`, `PhadeDev/library-file-plan-manager`.
- Users can also add ad hoc screen tabs beyond the presets via "+ Add screen" in the
  UI — these get deleted from the KV store (with a confirm prompt) via the same
  page's Delete button, pinned/preset tabs cannot be deleted.
